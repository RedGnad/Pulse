    ...
    if (isGovQuery) {
      try {
        ...
        if (active.length > 0) {
          ...
          govContext = `\n\n[GOVERNANCE] No proposals in voting period on ${network === "mainnet" ? "mainnet" : "testnet"}. Tell the user concisely that you can: 1) list active proposals ("show proposals"), 2) explain any proposal ("explain proposal #ID"), 3) vote on-chain ("vote yes/no/abstain on proposal #ID") via PulseGov. Keep it short and friendly.\n`;
        }
